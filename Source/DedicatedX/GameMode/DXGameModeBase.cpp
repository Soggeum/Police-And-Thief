...
void ADXGameModeBase::OnMainTimerElapsed()
{
	...
	{
		DXGameState->RemainingTime--;

		UE_LOG(LogTemp, Warning, TEXT("Remaining Time : %d"), DXGameState->RemainingTime)
		...
		if (DXGameState->RemainingTime <= 0)
		{
			DXGameState->MatchState = EMatchState::Ending;
			for (TObjectPtr<ADXPlayerController> PC : AlivePlayerControllers)
			{
				PC->ClientRPCShowGameResultWidget(AlivePlayerControllers.Num());
			}
		}
...
void ADXGameModeBase::Logout(AController* Exiting)
{
	...
	if (IsValid(ExitingPlayerController) && AlivePlayerControllers.Find(ExitingPlayerController) != INDEX_NONE)
	{
